...
import { redisClient, getOrSetCache, invalidateCache, invalidatePattern } from '../config/redis.js';
...
// @desc    Signup new user (OTP stored in Redis)
// @route   POST /api/v1/auth/signup
// @access  Public
export const signup = async (req, res) => { 
  const t = await sequelize.transaction();
  ...
  try {
    ...
    // ✅ 3. Generate OTP & Save to Redis (NOT SQL)
    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    const expirySeconds = 600; // 10 minutes
    
    // Key format: otp:user@email.com
    const redisKey = `otp:${email.toLowerCase().trim()}`;
    await redisClient.setEx(redisKey, expirySeconds, otp);

    // Optional: Clear SQL OTP fields for cleanliness (migration safety)
    user.password_reset_token = null;
    user.password_reset_expires = null;
    await user.save({ transaction: t });
    ...
    try {
      ...
      logger.error('Failed to send verification email:', emailErr);
      // Proceed anyway, user can request resend
    }
...
export const login = async (req, res) => {
  try {
    ...
    // 🚀 Invalidate specific user cache so next fetch gets fresh data
...
// @desc    Update current user (INVALIDATES CACHE)
// @route   PATCH /api/v1/auth/updateMe
// @access  Private
export const updateMe = async (req, res) => {
  const t = await sequelize.transaction();
...
// @desc    Resend Verification OTP (Stored in Redis)
// @route   POST /api/v1/auth/resend-otp
// @access  Public
export const resendVerificationOTP = async (req, res) => {
  const t = await sequelize.transaction();
  try {
    ...
    // Clear SQL OTP fields just in case
    user.password_reset_token = null;
    user.password_reset_expires = null;
    await user.save({ transaction: t });
    await t.commit();
    ...
    // ✅ Return calculated expiry time for frontend timer
...
// @desc    Verify OTP and Activate Account (Checks Redis)
// @route   POST /api/v1/auth/verify-otp
// @access  Public
export const verifyOTP = async (req, res) => {
  const t = await sequelize.transaction();
  try {
    ...
    // Find User
    const user = await User.findOne({ where: { email: cleanEmail }, transaction: t });
    if (!user) {
      await t.rollback();
      return res.status(404).json({ status: 'fail', message: 'User not found' });
    }
...
// @desc    Get all users (Admin - CACHED)
    ...
    const users = await getOrSetCache(
      ...
      async () => {
        ...
        return await User.findAll({
          attributes: ['id', 'name', 'email', 'role', 'phone', 'is_verified', 'created_at'],
