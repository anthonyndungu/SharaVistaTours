...
import { Review, TourPackage, User, Booking, sequelize } from '../models/index.js';
import { Op } from 'sequelize';
import logger from '../utils/logger.js';
// ✅ Import Redis helpers
import { getOrSetCache, invalidateCache, invalidatePattern } from '../config/redis.js';

// @desc    Create review (INVALIDATES CACHE)
// @route   POST /api/v1/reviews
// @access  Private
export const createReview = async (req, res) => {
  const t = await sequelize.transaction();
  ...
  try {
    ...
    // 🚀 INVALIDATE CACHE:
    // 1. Clear reviews list for this package
    await invalidatePattern(`reviews:package:${package_id}:*`);
    // 2. Clear package details cache (to update average rating)
    await invalidateCache(`package:single:${package_id}`);
    await invalidatePattern('packages:*');
    // 3. Clear global stats
    await invalidateCache('reviews:stats:global');
...
// @desc    Get all reviews for a package (CACHED)
// @route   GET /api/v1/packages/:packageId/reviews
// @access  Public
export const getPackageReviews = async (req, res) => {
  try {
    const { packageId } = req.params;
    // Create cache key based on params AND query filters
    const queryString = JSON.stringify(req.query);
    const cacheKey = `reviews:package:${packageId}:${queryString}`;
...
// @desc    Update review (INVALIDATES CACHE)
// @route   PATCH /api/v1/reviews/:id
// @access  Private
export const updateReview = async (req, res) => {
  const t = await sequelize.transaction();
  try {
    ...
    // 🚀 INVALIDATE CACHE
    await invalidatePattern(`reviews:package:${review.package_id}:*`);
    await invalidateCache(`package:single:${review.package_id}`);
    await invalidatePattern('packages:*');
    await invalidateCache('reviews:stats:global');
...
// @desc    Delete review (INVALIDATES CACHE)
// @route   DELETE /api/v1/reviews/:id
// @access  Private
export const deleteReview = async (req, res) => {
  const t = await sequelize.transaction();
  try {
    ...
    // 🚀 INVALIDATE CACHE
    await invalidatePattern(`reviews:package:${packageId}:*`);
    await invalidateCache(`package:single:${packageId}`);
    await invalidatePattern('packages:*');
    await invalidateCache('reviews:stats:global');
...
// @desc    Get review statistics (CACHED)
// @route   GET /api/v1/reviews/stats
// @access  Public
export const getReviewStats = async (req, res) => {
  try {
    const stats = await getOrSetCache(
      ...
      async () => {
        logger.debug('Cache MISS for reviews:stats:global. Fetching from DB...');
