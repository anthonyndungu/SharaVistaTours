...
import { getOrSetCache, invalidatePattern } from '../config/redis.js';
...
// @desc    Get all tour packages (CACHED)
// @route   GET /api/v1/packages
// @access  Public
export const getAllPackages = async (req, res) => {
  logger.info(`getAllPackages called with query ${JSON.stringify(req.query)}`);
  ...
  try {
    ...
    const cacheKey = `packages:list:${queryString}`;

    const result = await getOrSetCache(
      cacheKey,
      async () => {
        // 🟡 This function runs ONLY if cache misses (DB Fetch)
        ...
        const { count, rows } = await TourPackage.findAndCountAll(queryOptions);
        ...
        return {
          ...
          page,
          pages: Math.ceil(count / limit),
          packages: rows // This is the critical array
        };
      },
      1800 // Cache for 30 minutes
    );

    // ✅ Double check result before sending
    if (!result || !result.packages) {
      logger.warn('Cache returned invalid data structure, returning empty list');
    ...
    logger.info(`Returning ${result.results} packages`);

    // ✅ Send response matching frontend expectation
    res.status(200).json({
      status: 'success',
      ...result
    });
...
// @desc    Get single tour package (CACHED)
// @route   GET /api/v1/packages/:id
// @access  Public
export const getPackageById = async (req, res) => {
  try {
    const cacheKey = `package:single:${req.params.id}`;

    const packageData = await getOrSetCache(
      cacheKey,
      async () => {
        logger.debug(`Cache MISS for ${cacheKey}. Fetching from DB...`);
...
// @desc    Create new tour package (INVALIDATES CACHE)
// @route   POST /api/v1/packages
// @access  Private/Admin
export const createPackage = async (req, res) => {
  logger.info('createPackage called', { body: req.body });
  ...
  try {
    ...
    // 🚀 INVALIDATE CACHE: Clear all package-related caches
    await invalidatePattern('packages:*');
...
// @desc    Update tour package (INVALIDATES CACHE)
// @route   PATCH /api/v1/packages/:id
// @access  Private/Admin
export const updatePackage = async (req, res) => {
  logger.info('updatePackage called', { params: req.params, body: req.body });
  ...
  try {
    ...
    // 🚀 INVALIDATE CACHE: Clear specific item and all lists
    await invalidatePattern('packages:*');
...
// @desc    Delete tour package (INVALIDATES CACHE)
// @route   DELETE /api/v1/packages/:id
// @access  Private/Admin
export const deletePackage = async (req, res) => {
  const t = await sequelize.transaction();
...
// @desc    Get package statistics (CACHED)
// @route   GET /api/v1/packages/stats
// @access  Private/Admin
export const getPackageStats = async (req, res) => {
  try {
    const stats = await getOrSetCache(
      'packages:stats:global',
      async () => {
        logger.debug('Cache MISS for packages:stats:global. Fetching from DB...');
