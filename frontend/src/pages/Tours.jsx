...
export default function Tours() {
  ...
  useEffect(() => {
    ...
    if (t) {
      ...
      setSelectedType(found || { value: t, label: t.charAt(0).toUpperCase() + t.slice(1).replace(/-/g, ' ') });
    } else {
      setSelectedType(null);
    }

    if (d) {
      const found = uniqueDestinations.find(o => o.value === d);
      setSelectedDest(found || { value: d, label: d.charAt(0).toUpperCase() + d.slice(1).replace(/-/g, ' ') });
    } else {
      setSelectedDest(null);
    }
  }, [location.search, uniqueTypes, uniqueDestinations]);
