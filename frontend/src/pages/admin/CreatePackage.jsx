...
export default function CreatePackage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // In Create mode, 'existing' should always be false, but we map safely
      const imagesPayload = imagePreviews.map((img) => ({
        id: null, // Always null for new packages
      ...
      // Key 'newImages' MUST match your multer middleware: upload.array('newImages')
      ...
      // Note: We pass formDataToSend directly
      const resultAction = await dispatch(createPackage(formDataToSend)).unwrap();
