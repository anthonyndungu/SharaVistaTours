...
function LocationPicker({ onLocationSelect, initialLat, initialLng }) {
  ...
  const map = useMap(); // Hook to control map view
  ...
  const handleSuggestionClick = async (item) => {
    ...
    // 3. Update Form Inputs Instantly
    // We pass the display_name immediately so the user sees feedback instantly
    onLocationSelect(lat, lon, display_name);

    // 4. Clear UI
    setSuggestions([]);
    setSearchQuery(display_name); // Optional: Set input value to selected place name

    // 5. (Optional) Fetch full detailed address via Reverse Geocoding
    // Sometimes the search result name is short (e.g., "Nairobi"), this gets the full street address
    setIsFetchingAddress(true);
    try {
