          ...
          <nav style={{ padding: '10px 0', flex: 1 }}>
            {navigation.map((item) => (
              <Link
                ...
                onMouseEnter={(e) => {
                  if (location.pathname !== item.href) e.currentTarget.style.backgroundColor = COLORS.hoverBg;
                }}
                onMouseLeave={(e) => {
                  if (location.pathname !== item.href) e.currentTarget.style.backgroundColor = 'transparent';
                }}
              >
                <span style={{ marginRight: '12px', fontSize: '18px' }}>{item.icon}</span>
                {item.name}
